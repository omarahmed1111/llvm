...
inline pi_result ur2piDeviceInfoValue(zer_device_info_t ParamName,
...
inline pi_result piContextCreate(const pi_context_properties *properties,
  ...
  auto hDevicesConst = reinterpret_cast<const zer_device_handle_t *>(devices);
  auto hDevices = const_cast<zer_device_handle_t *>(hDevicesConst);
  auto phContext = reinterpret_cast<zer_context_handle_t *>(retcontext);

  HANDLE_ERRORS(zerContextCreate(num_devices, hDevices, phContext));
...
inline pi_result piContextGetInfo(pi_context context,
  ...
  static std::unordered_map<pi_context_info, zer_context_info_t> InfoMapping = {
      {PI_CONTEXT_INFO_NUM_DEVICES, ZER_CONTEXT_INFO_NUM_DEVICES},
      {PI_CONTEXT_INFO_DEVICES, ZER_CONTEXT_INFO_DEVICES},
      {PI_CONTEXT_INFO_REFERENCE_COUNT,
       (zer_context_info_t)ZER_EXT_CONTEXT_INFO_REFERENCE_COUNT},
      {PI_CONTEXT_INFO_ATOMIC_MEMORY_ORDER_CAPABILITIES,
       (zer_context_info_t)
           ZER_EXT_CONTEXT_INFO_ATOMIC_MEMORY_ORDER_CAPABILITIES},
      {PI_CONTEXT_INFO_ATOMIC_MEMORY_SCOPE_CAPABILITIES,
       (zer_context_info_t)
           ZER_EXT_CONTEXT_INFO_ATOMIC_MEMORY_SCOPE_CAPABILITIES},
      {PI_EXT_ONEAPI_CONTEXT_INFO_USM_MEMCPY2D_SUPPORT,
       (zer_context_info_t)ZER_EXT_CONTEXT_INFO_USM_MEMCPY2D_SUPPORT},
      {PI_EXT_ONEAPI_CONTEXT_INFO_USM_FILL2D_SUPPORT,
       (zer_context_info_t)ZER_EXT_CONTEXT_INFO_USM_FILL2D_SUPPORT},
      {PI_EXT_ONEAPI_CONTEXT_INFO_USM_MEMSET2D_SUPPORT,
       (zer_context_info_t)ZER_EXT_CONTEXT_INFO_USM_MEMSET2D_SUPPORT},
  };
  ...
  auto hContext = reinterpret_cast<zer_context_handle_t>(context);
  HANDLE_ERRORS(
      zerContextGetInfo(hContext, InfoType->second, &SizeInOut, param_value));

  if (param_value_size_ret) {
    *param_value_size_ret = SizeInOut;
  }
...
inline pi_result piContextRetain(pi_context context) {
  auto hContext = reinterpret_cast<zer_context_handle_t>(context);
  HANDLE_ERRORS(zerContextGetReference(hContext));
...
inline pi_result piContextRelease(pi_context context) {
  auto hContext = reinterpret_cast<zer_context_handle_t>(context);
  HANDLE_ERRORS(zerContextRelease(hContext));
